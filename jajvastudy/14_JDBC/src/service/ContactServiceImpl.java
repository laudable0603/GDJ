...
public class ContactServiceImpl implements ContactService {
	...
	/********************* Field **********************************/
	// DAO에 데이터를 전달하고, DAO로 부터 결과를 반환 받기 위해서 DAO를 선언
	private ContactDAO dao = ContactDAO.getInstanse();

	/********************* Method **********************************/

	@Override
	public void addContact(ContactDTO contact) {
		int result = dao.insertContact(contact); // 들어온 contact정보를 ContactDAO의 insertContact에 전달
		if (result > 0) {
	...
	public void modifyContact(ContactDTO contact) {
		int result = dao.updateContact(contact); // 들어온 contact정보를 ContactDAO의 updateContact에 전달
		if (result > 0) {
	...
	public void deleteContact(int contact_no) {
		int result = dao.deleteContact(contact_no); // 들어온 contact_no정보를 ContactDAO의 deleteContact에 전달
		if (result > 0) {
	...
	public void findContactByNo(int contact_no) {
		...
		if (contact == null) {
			System.out.println("해당 번호로 등록된 연락처가 없습니다.");
		} else {
	...
	public void findAllContacts() {
		...
		if (contacts.isEmpty()) {
			...
			/*
			 * for(ContatDTO contact : contacts){ System.out.println(contact); }
			 */

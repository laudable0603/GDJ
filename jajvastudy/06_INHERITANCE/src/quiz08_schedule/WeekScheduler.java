...
public class WeekScheduler {
		...
		for (int i = 0; i < week.length; i++) {
			if (dayName.equals(dayNames[i])) {
				if (week[i] == null) { // 등록된 스케줄이 없으면
					System.out.print("스케줄 내용 입력 >>> ");
					String schedule = sc.nextLine(); // 스케줄에 공백 입력이 가능해짐.
					Day day = new Day();
	...
	private void changeSchedule() {
		...
		System.out.println("변경할 요일을 입력해주세요 >>>  ");
		String dayName = sc.next().substring(0, 1);
		sc.nextLine();
		for (int i = 0; i < week.length; i++) {
			if (dayName.equals(dayNames[i])) {
				Day day = new Day();
				if (week[i] == null) {
					System.out.println(dayName + "요일의 스케줄이 없습니다.");
					System.out.println("새 스케줄을 등록할까요? [Y / N] >>> ");
					String yesNo = sc.next().substring(0, 1);
					sc.nextLine();
					if (yesNo.equalsIgnoreCase("y")) {
						System.out.println("등록할 스케줄을 입력해주세요.");
					...
					System.out.print("스케줄을 변경할까요? [ Y / N ] >>>  ");
					String yesNo = sc.next().substring(0, 1);
					sc.nextLine();
					if (yesNo.equalsIgnoreCase("y")) {
						...
						String schedule = sc.nextLine();
						day.setSchedule(schedule);
						System.out.println(dayName + "의 새 스케줄이 등록되었습니다.");
		...
		System.out.println(dayName + "요일은 없는요일 입니다.");
		...
		for (int i = 0; i < week.length; i++) {
			if (dayName.equals(dayNames[i])) {
				if (week[i] == null) {
					...
					if (yesNo.equalsIgnoreCase("y")) {
						week[i] = null;
						System.out.println(dayName + "요일의 스케줄이 삭제되었습니다.");
					} else {
	...
	public void manage() {
		while (true) {
			System.out.print("1.등록  2.수정  3.삭제  4.전체조회  0.프로그램종료   >>>");

...
/*
	...
	4. URL						   컨텍스트 = 프로젝트 = 어플리케이션, URLMapping은 내가 추가한 Servlet과 같다.
...
public class HelloServlet extends HttpServlet { // Servlet은 HttpServlet을 상속 받으면 된다.
	...
	/*
	  3. 서비스
	  	1) 클라이언트의 요청마다 매번 호출
	  	2) GET 방식의 쵸청은 doGet() 메소드 호출, POST 방식의 요청은 doPost()메소드 호출
	 */
	...
	/*
	  4.doGet
	  	...
	  	2) 요청 메소드가 GET인 경우
	  		(1) $.ajax({
	  				'type':'GET',
	  				'url':'href="http://localhost:9090/01_servlet/HelloServlet'
	  			});
	  		(2) <a href="http://localhost:9090/01_servlet/HelloServlet">링크</a>
	  		(3) <form method="GET" action="href="http://localhost:9090/01_servlet/HelloServlet"> 또는 <form>
	...
	/*
	 	5. doPost
	 		...
	 		2) 요청 메소드가 POST인 경우
	 			...
	 			(2) <form method="POST" action="href="http://localhost:9090/01_servlet/HelloServlet">

...
public class BoardDetailService implements BoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 요청 파라미터
		String str = request.getParameter("board_no");		//파라미터 board_no이 온다
		Optional<String> opt = Optional.ofNullable(str);	//파라미너가 null일 수 있다.
		int board_no = Integer.parseInt(opt.orElse("0"));   //파라미터가 null이면 "0" 쓰라

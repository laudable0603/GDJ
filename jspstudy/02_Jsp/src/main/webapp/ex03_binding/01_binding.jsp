...
<%-- 
	1. Jsp의 Binding 영역 4개
		...
		3) request     : HttpSerlvetRequest와 같은 영역
...
<%--
	2. 우선 순위
		1) 같은 이름의 속성이 서로 다른 영역에 저장된 경우 먼저 사용되는 우선 순위가 있다.
 ...
 --%>
<%
	application.setAttribute("movie", "기생충");    // 우선 순위 제일 낮음
	session.setAttribute("movie", "터미네이터");
	request.setAttribute("movie", "아바타");
	pageContext.setAttribute("movie", "미나리");	// 우선 순위 제일 높음
%>
<!--<div>application's movie : ${movie}</div>  해당처럼 작성하면 pageContext값만 인식 -->
...
<div>pageContext's movie : ${pageScope.movie}</div>

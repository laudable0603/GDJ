...
<script>
   ...
   function fn_getMember(){
	   ...
	   $('body').on('click','.btn_detail',function(){
		  // alert($(this).prev().val());//이전 인접 형제(prev), 이후 인접 형제(next)
		   $.ajax({
			   /* 요청 */
			  type: 'get',
   ...
   function fn_registration(){
	   ...
	   $('#btn_add').click(function(){
		   $.ajax({
			   ...
			   data: $('#frm_member').serialize(), // serialize(): 폼의 모든 입력 요소(name속성)를 파라미터로 변환
			   /* 응답 */
			   dataType: 'json',
			   //정상응답
...
<body>
	...
	<div class="wrap">
		...
		<form id="frm_member">
			...
			<div class="btn_area">
				<input type="button" value="초기화" id="btn_init" onclick="fn_init();" class="btn_primary">

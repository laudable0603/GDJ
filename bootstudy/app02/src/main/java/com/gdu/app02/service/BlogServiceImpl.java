...
public class BlogServiceImpl implements BlogService {
	...
	public void getBlogList(Model model) {
		// Model에 저장된 request 꺼내기	(* 권장사항 아님, 굳이 할 필요없다. )
		Map<String, Object> modelMap = model.asMap();	// model을 map으로 변신
		HttpServletRequest request = (HttpServletRequest)modelMap.get("request");	//* 리퀘스트에 전달된 값은 컨트로럴에서 ~
	...
	public void saveBlog(HttpServletRequest request, HttpServletResponse response) {
		...
		// 작성자의 IP
		// 작성된 내용이 어딘가를 경유해서 도착하면 원래 ip가 X-Fowarded-For라는 요청헤더에 저장된다.
		// 출발						도착
		// 1.1.1.1					1.1.1.1 : request.getRemoteAddr()
		//							null	: request.getHeader("X-Fowarded-For")		* 경유지가 없을 경우 null값
		// 출발			경유		도착
		// 1.1.1.1		2.2.2.2		2.2.2.2 : request.getRemoteAddr()
		// 							1.1.1.1 : request.getHeader("X-Fowarded-For")

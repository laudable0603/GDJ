...
public class UploadServiceImpl implements UploadService {
	...
	public void save(MultipartHttpServletRequest multipartRequest, HttpServletResponse response) {
		...
		//System.out.println(upload); // uploadNo없음
		
		// DB에 UploadDTO 저장
		int uploadResult = uploadMapper.insertUpload(upload); //xml -> <selectKey>에 의해서 인수 upload에 uploadNo값이 저장된다.
		
		//System.out.println(upload);  // uploadNo있음
		/* ATTACH 테이블에 저장하기 */
		
		
		// 첨부된 파일 목록
		List<MultipartFile> files = multipartRequest.getFiles("files"); // <input type="file" name="files">
		System.out.println(files);//첨부된 목록의 파일
		
		
		// 첨부결과
		int attachResult;
		if(files.get(0).getSize() ==0) { //첨부가 없는 경우 files.size()가 1이다. 그래서
			attachResult = 1;
		} else {
			attachResult = 0;
		}
		...
		for(MultipartFile multipartFile : files) {
			...
			try {
				...
				if(multipartFile != null && multipartFile.isEmpty() == false) { //둘 다 필요함
					...
					origin = origin.substring(origin.lastIndexOf("\\") + 1); // IE는 origin에 전체 경로가 붙어서 파일명만 사용해야함

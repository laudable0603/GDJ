...
public class BoardDAO {
	...
	public BoardDAO() {
		...
		try {
			...
			dataSource = (DataSource)context.lookup("java:comp/env/jdbc/oracle11g");//java:comp/env/Resource의 name
	...
	//private 메소드
	// 이 메소드는 boardDAO에서만 사용한다.(메소드 마다 끝에 작성)
	private void close() {
		try {
			if(rs != null) {rs.close();}
			if(ps != null) {ps.close();}
	...
	public List<BoardDTO> selectAllBoards(){
		...
		try {
			con = dataSource.getConnection(); //Connection Pool에서 Connection 하나를 얻어온다.
			sql = "SELECT BOARD_NO, TITLE, CONTENT, WRITER, CREATE_DATE, MODIFY_DATE FROM BOARD ORDER BY BOARD_NO DESC";
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while(rs.next()){//넥스트 호출 1회당 쿼리문 1행씩 진행 검색된 데이터가 없으면 false 있으면 true

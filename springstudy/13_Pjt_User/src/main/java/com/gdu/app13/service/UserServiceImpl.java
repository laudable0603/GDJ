...
public class UserServiceImpl implements UserService {
	...
	public void keepLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		if (keepLogin != null) {
			...
			UserDTO user = UserDTO.builder().id(id).sessionId(sessionId) // 밀리초를 위해 1000을 더 곱한다.(15일 후를 뜻함)
					.sessionLimitDate(new Date(System.currentTimeMillis() + 1000 * 3600 * 24 * 15)).build();
	...
	@Override
	public String getNvaerLoginApiURL(HttpServletRequest request) {
		String apiURL = null;
		try {

			String clientId = "MfHKZlNOIPwPkhpvP42V";// 애플리케이션 클라이언트 아이디값";
			String redirectURI = URLEncoder
					.encode("http://localhost:9090" + request.getContextPath() + "/user/naver/login", "UTF-8"); // 네이버로그인
																												// CALLBACK에
																												// 해당 주소
																												// 입력
	...
	@Override
	public UserDTO getNaverLoginTokenProfile(HttpServletRequest request) {
		String clientId = "MfHKZlNOIPwPkhpvP42V";// 애플리케이션 클라이언트 아이디값";
		String clientSecret = "i1ZAky8pkC";// 애플리케이션 클라이언트 시크릿값";
		...
		StringBuffer res = new StringBuffer(); // StringBuffer는 StringBuilder와 동일한 역할수행
		try {
			String apiURL;
			...
			/*
				  {
				  		"access_token":"AAAAN5ZqWv5waeMt_iftCNKC56l2ajI2kyov1Gtch-JpiAWCZABxOeRCO9l15aiSPPjoN-vjW5cBeH8STVuQHoXiT-0",
				  		"refresh_token":"LHATOiskdisIisiseOvV8xPXuy9OVtiikYBgCkEDTr48NRSgC2EquQvp9QFFvdsdNWmPjlZipgDYMMYwPkwstRTiijFIh9TWm2GwEQ4PKxvITLXmZz33wPwlisisWTSX7ip5VDvsJL",
				  		"token_type":"bearer",
				  		"expires_in":"3600"
				  }
			 */
		...
		try {
		   ...
		   int responseCode = con.getResponseCode();
			...
			/*
			 sb.toString();//사용자의 프로필 정보
			 {
				 "resultcode":"00"
				 ,"message":"success"
				 ,"response":{
					 "id":"asi1i31iosod3QASDsa-IDOdi-2xwQ"
					 ,"gender":"M"
					 ,"email":"qwe1234@naver.com"

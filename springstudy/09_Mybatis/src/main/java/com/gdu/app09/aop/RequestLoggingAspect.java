...
public class RequestLoggingAspect {
	...
	//포인트컷 설정									// 컨트롤러의 모든 메소드를 포인트컷으로 지정
	@Pointcut("within(com.gdu.app09.controller..*)")//controller 패키지 아래 모든 패키지의 모든 클래스
													//컨트롤러의 모든 메소드에서 어드바이스(콘솔에 로그 직기)가 동작한다.
	...
	public Object executeLogging(ProceedingJoinPoint joinPoint) throws Throwable { //@Around는 반드시 ProceedingJoinPoint 실행해야함
		
		// HttpServletRequest를 사용하는 방법
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		
		// HttpServletRequest를 Map으로 바꾸기
		// 파라미터는 Map의 key가 되고, 값은 Map의 vlaue가 된다.

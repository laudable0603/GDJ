...
<div>
	...
	<span id="btn_comment_list">
		댓글
		<span id="comment_count" style="color: blue"></span>개
	</span>
	...
	<div>
		<form id="frm_add_comment">
			<div class="add_comment">
				...
				<div class="add_comment_btn add_comment_input">
					<input type="text" name="content" id="content" placeholder="댓글을 작성하려면 로그인 해 주세요">
					<input type="button" value="작성" id="btn_add_comment">
				</div>
			</div>
			<input type="hidden" name="blogNo" value="${blog.blogNo}">
		</form>
					...
					*/
					...
					$.each(resData.commentList, function(i, comment){
						...
						if(comment.state == 1) {
							...
							//댓글에만 답글을 달 수 있도록 if 처리 필요 depth가 0이면
							if(comment.depth == 0){
		...
		function fn_addReply(){
			$(document).on('click', '.btn_reply_add', function(){
				...
				$.ajax({
					...
					//버튼기준으로 잡아준다.
					// 해당코드는 안됨$('.frm_reply').serialize(),
					// closest => 가장 가까운 '  ' 해당 객체

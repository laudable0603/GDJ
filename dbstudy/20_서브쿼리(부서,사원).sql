...
/*
    ...
    4. 실행 순서는 서브쿼리 -> 메인쿼리, 서브쿼리 실행 결과를 메인쿼리에서 사용함
    5. 사용되는 절에 따른 구분
        1) SELECT절 : 스칼라 서브쿼리 -단일행
        2) FROM절   : 인라인뷰 - 다중행
        3) WHERE절  : 서브쿼리 - 단일행, 다중행
    6. 서브쿼리 결과에 따른 구분
        1) 단일 행 서브쿼리
            (1) 서브쿼리 결과가 1개
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION = (SELECT POSITION        --단일 행 서브쿼리이므로 연산자를 =를 사용, 하위쿼리는 메인쿼리와 동등비교(=) 되므로 반드시 POSITION을 반환
                     FROM EMPLOYEE
                    WHERE EMP_NO = 1001); --EMP_NO PK이므로 단일행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION IN(SELECT POSITION     -- 다중 행 서브쿼리의 동등비교는 IN연산으로 수행해야 함.
                     FROM EMPLOYEE
...
-- 어떤 급여(2000000, 2000000)이든 하나의 급여보다 많이 받으면 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
...
-- 모든 급여(2000000, 2000000)와 비교해서 많이 받으면 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
...
-- 스칼라 서브쿼리 접근
-- 스칼라 서브쿼리에 경우에는 일치하지 않는 데이터는 NULL처리 하기때문에
...
/* FROM절의 서브쿼리*/ --뷰는 테이블과 같다 테이블은 원본, 뷰는 복사본같은 느낌
/*
    인라인뷰(Inline-View)
    ...
    2. 인라인뷰는 주로 테이블 형식임
    3. 인라인뷰에 별명을 주고 사용함
    4. 인라인뷰에서 조회한 칼럼만 메인쿼리에서 조회할 수 있음
    5. SELECT문의 실행순서를 바꿀 때 사용
    
    -- 모든 목록보기는 정렬 후 가져와야하는데 
    -- 셀렉트문을 먼저 실행하기 위해 인라인뷰를 사용할 때가 많다.
*/
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFGRAABAAALDBAAC';  
...
-- 3 :SELECT
-- 1 :  FROM (ROWNUM의 별명지정하기)
-- 2 : WHERE ROWNUM의 별명사용하기
...
-- 1) ROWNUM칼럼 함수를 안쓰는 오리지날
SELECT B.ROW_NUM, B.EMP_NO, B.NAME, B.SALARY
  FROM(SELECT ROWNUM AS ROW_NUM, A.EMP_NO, A.NAME, A.SALARY
        FROM (SELECT EMP_NO, SALARY, NAME
                FROM EMPLOYEE
               ORDER BY SALARY DESC) A)B
 WHERE B.ROW_NUM BETWEEN 2 AND 2;  --목록보기 예시               

    
-- 2) ROW_NUMBER() 함수
--  정렬과 행 번호 추가를 동시에 진행하는 함수
SELECT A.EMP_NO, A.NAME, A.SALARY
  FROM(SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS ROW_NUM, EMP_NO, NAME, SALARY --ROW_NUMBER는 별명을 줘야된다
         FROM EMPLOYEE) A
 WHERE A.ROW_NUM BETWEEN 2 AND 2;
 
-- 3) RANK() 함수
--      정렬 후 순위 매기는 함수
--      목록 가져오기에서는 부적절(동점자 처리 때문에 가져오는 목록의 수가 정확하지 못하다)
SELECT A.EMP_NO, A.NAME, A.SALARY
...
UPDATE EMPLOYEE2
   SET NAME = (SELECT NAME FROM EMPLOYEE WHERE EMP_NO =1004) -- 1004번의 이름을 가져와서 1001번에 넣어줘라
...
DELETE
  FROM EMPLOYEE2
 WHERE DEPART IN (SELECT DEPT_NO  -- 부서번호 IN (영업부의 부서번호)
                     FROM DEPARTMENT2
                    WHERE DEPT_NAME = '영업부');

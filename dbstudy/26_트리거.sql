/*
    ...
    4. 기본적으로 작업 수행되는 행(ROW) 단위로 트리거가 적용됨
    5. 형식
...
CREATE OR REPLACE TRIGGER TRIG2
    AFTER
    INSERT OR UPDATE OR DELETE --작성 순서 상관없음
    ON DEPARTMENT
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
...
-- 1) 퇴사자 테이블 생성(EMPLOYEES 테이블과 동일한 구조, 데이터 X 복사)
DROP TABLE RITIRES;
CREATE TABLE RITIRES
    AS (SELECT * FROM EMPLOYEES WHERE 1 = 2);

-- 2) RETIRE_ID, RETIRE_DATE 칼럼 추가하기
ALTER TABLE RITIRES ADD RETIRE_ID NUMBER NOT NULL;
ALTER TABLE RITIRES ADD RETIRE_DATE DATE;
...
BEGIN
    INSERT INTO
        RITIRES(RETIRE_ID, RETIRE_DATE, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID)
...
END RETIRE_TRIG;
-- 6) EMPLOYEES 테이블의 데이터 DELETE 수행하기 (RETIRE_TRIG 동작확인)

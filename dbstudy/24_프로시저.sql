...
IS
    FNAME EMPLOYEES.FIRST_NAME%TYPE; --EMPLOYEES테이블에 FIRST_NAME 과 같은 타입
BEGIN
    SELECT FIRST_NAME  --FIRST_NAME 칼럼 값을
      INTO FNAME       --FNAME이라는 변수에 저장
      FROM EMPLOYEES
...
--프로시저 PROC4 정의
-- 사원번호=100인 사원의 FISRT_NAME을 출력 파라미터 FNAME에 저장하기
...
EXCEPTION
    WHEN OTHERS THEN -- 모든 예외를 처리함(WHEN NO_DATA_FOUND THEN가능)
        FNAME := 'NoNAME';
END PROC5;

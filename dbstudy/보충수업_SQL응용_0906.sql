...
-- 프로시저 작성 -- 프로시저한테 주면 IN 뱉으면 OUT
-- 학과번호(DEPTNO) 전달하면 소속 학생, 교수, 학과(데이터) 모두 제거하는 프로시저
-- 프로시저 실행은 학과번호 101번인 학과를 대상으로 실행
-- 프로시저 이름은 MY_PROC
-- 예외 발생시 예외코드 + 예외 메시지 출력 + ROLLBACK
...
-- 아이디 'Dals'을 전달하면 학번 '20101'이 반환되는지 확인
-- 함수이름 GET_STUDENT
...
-- 트리거
-- SAMPLE 테이블의 행을 갱신, 삭제 이후 하면 'SampleTrg동작' 서버메시지 출력하기
CREATE OR REPLACE TRIGGER SampleTrg
    ...
    FOR EACH ROW
BEGIN
    DBMS_OUTPUT.PUT_LINE('SampleTrg동작');
END SampleTrg;

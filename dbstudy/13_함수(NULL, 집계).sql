...
-- 1. NVL
--    NVL(칼럼, 칼럼값이 NULL일 때 NULL대신 사용할 값)
...
SELECT
     ...
     , SUM(ENG) -- 영어합
     , SUM(MATH) -- 수학합
--     , SUM(KOR, ENG, MATH)  --인수(arguments)가 3개이므로 불가능함 (인수가 1개여야함)
     , SUM(KOR + ENG + MATH)    --KOR + ENG + MATH 와 같은 연산(SUM 함수를 잘못 사용한 예시) ROW단위 연산

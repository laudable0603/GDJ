/*
    ...
    6. 인덱스 생성
        1) 비고유 인덱스 (칼럼 내용에 중복이 있는 경우) -- 주로 사용
            CREATE INDEX 인덱스_이름 ON 테이블_이름(칼럼_이름);
        2) 고유 인덱스 (칼럼 내용에 중복이 없는 경우) -- 쓰일 일이 별로 없음
            CREATE UBIQUE INDEX 인덱스_이름 ON 테이블_이름(칼럼_이름);
*/
...
-- 인덱스 생성
CREATE INDEX IDX_NAME ON EMPLOYEE(NAME); --이름이 자주 조회되는 데이터면 해당처럼 수정
                                         -- 단, 수정이 자주되는 데이터면 안하는게 좋음

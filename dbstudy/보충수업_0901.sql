...
--2) 서브쿼리 
-- 서브쿼리 : LOCATION_ID 가 1000~ 1500인 지역의 COUNTRY_ID 가져오기(다중행)
-- 메인쿼리 : SELECT 칼럼 FROM COUNTRIES WHERE COUNTRY_ID = (서브쿼리)
...
-- 서브쿼리 : MANGER_ID의 중복을 제거 결과(MANAGER의 번호 모아두기)
--       ㄴ 서브쿼리에 결과에 NULL이포함되어있으면 메인쿼리가 동작하지 않는다
...
-- CITY, LOCATION_ID    -LOCATION_ID, DEPARTMENT_ID - DEPARTMENT_ID, EMPLOYEE_ID,
--          (PK)          (FK)          (PK)            (FK)
...
-- 조건 : 근무중인사원수 = 최대사원수
-- HAVING절 사용
SELECT A.DEPARTMENT_ID, A.ROW_NUM
  FROM(SELECT ROW_NUMBER() OVER(ORDER BY COUNT(*) DESC) AS ROW_NUM, DEPARTMENT_ID
         FROM EMPLOYEES
        WHERE DEPARTMENT_ID IS NOT NULL
        GROUP BY DEPARTMENT_ID) A
 WHERE A.ROW_NUM BETWEEN 1 AND 1; --틀림
...
-- 인라인뷰 A : 가장 높은 연봉이 맨위에 있는 테이블
-- 인라인뷰 B : 연봉순으로 정렬된 테이블에 행 번호(RN)를 부착시켜둔 테이블
...
-- 인라인뷰 A : 연봉순으로 정렬된 뒤 행 번호(연봉순위) 가 부착된 테이블
SELECT A.연봉순위, A.EMPLOYEE_ID, A.FIRST_NAME, A.SALARY
  FROM(SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS 연봉순위, EMPLOYEE_ID, FIRST_NAME, SALARY
         FROM EMPLOYEES) A
 WHERE A.연봉순위 BETWEEN 11 AND 20;
/*
    ...
    4. 서버메시지 출력을 위해 최초 1회 설정이 필요(디폴트값이 SET SEVEROUTPUT OFF 상태이다.)
        -따라서, SET SERVEROUTPUT ON;
...
-- 2. 참조 변수 선언
-- 참조 변수 : 특정 칼럼의 타입을 그대로 사용하는 것
-- 선언 방법 
-- 테이블명.칼럼%TYPE

DECLARE
    --참조 변수 선언
    NAME EMPLOYEES.FIRST_NAME%TYPE;-- FIRST_NAME과 같은 타입으로 선언해달라
BEGIN
    NAME := '시나몬';
    DBMS_OUTPUT.PUT_LINE('내 이름은 ' || NAME || '입니다');
END;
